...
let CalendarService = CalendarService_1 = class CalendarService {
    ...
    async updateCalendarId(calendarId) {
        const normalizedCalendarId = calendarId.trim();
        if (!normalizedCalendarId) {
            throw new common_1.BadRequestException('calendarId is required.');
        }
        const config = await this.prisma.googleCalendarConfig.findFirst({
            orderBy: { createdAt: 'asc' },
        });
        if (!config) {
            throw new common_1.NotFoundException('Google Calendar is not connected.');
        }
        ...
        try {
            await this.executeCalendarCall(async () => ctx.calendar.calendars.get({ calendarId: normalizedCalendarId }));
        }
        catch (error) {
            if (this.isGoogleNotFound(error)) {
                throw new common_1.BadRequestException('The provided calendarId was not found or is not accessible by this Google account.');
            }
            throw error;
        }
        await this.prisma.googleCalendarConfig.update({
    ...
    buildBookingEventRequestBody(booking) {
        return {
